---
title: "DO-178C Requirements Traceability Matrix Template — Airborne Software Requirements (Free Download)"
description: "Close your DO-178C traceability gaps before DER review. Free RTM template with DAL annotations, bidirectional traceability, and derived requirements."
standard: do-178c
badge_color: "#6A1B9A"
clauses:
...
DO-178C (Software Considerations in Airborne Systems and Equipment Certification), the successor to DO-178B, is the standard by which the FAA and EASA certify that airborne software is safe to fly. The traceability requirements are structurally identical between DO-178B and DO-178C; the key additions in DO-178C are model-based (DO-331), object-oriented (DO-332), and formal-methods (DO-333) technology supplements. It is not a quality management standard; it is a certification basis. Every objective in DO-178C must be satisfied, and for most objectives, satisfaction is demonstrated through documented evidence. The requirements traceability matrix (RTM) is that evidence.
...
**§11.9 Traceability Data** is an explicit life cycle data item: a document showing the bidirectional trace from system requirements to HLRs to LLRs to source code to tests. The requirements traceability matrix is this document. At DER review, this is one of the first things examined.
...
## Related DO-178C templates and checklists

The RTM template covers §11.9 traceability data. These free templates cover adjacent DO-178C planning and risk artifacts:

- [Verification & Validation Plan template](/free-templates/vv-plan) — covers the Software Verification Plan structure (test strategy, review methods, pass/fail criteria) required by DO-178C §11.3.
- [FMEA template](/free-templates/fmea) — failure modes and effects analysis for hardware/software interaction analysis and common-cause failure assessment.
- [Hazard Log template](/free-templates/hazard-log) — tracks safety hazards identified during the system safety assessment, including derived requirements fed back from DO-178C analysis.
- [Browse all free templates →](/free-templates)

## From spreadsheet to live traceability

The RTM spreadsheet is a starting point. [RTMify Live](/live) turns it into a managed traceability tool: upload your workbook and get continuous requirement-to-test gap detection, impact analysis when requirements change, Design BOM sync, and MCP-based querying across your full trace chain. No migration required — RTMify Live ingests the same .xlsx format.
